...
2. Be sure to keep a browser window open to the excellent [Liquibase docs](http://www.liquibase.org/manual/home)


##Database Setup

1. Run the `gradlew createDatabaseScript` task, which will produce a script called `starth2` (on Mac and Linux) or `starth2.bat` (on Windows). You can then start the database by running this script from the command line, which keeps the database running in the background while you continue to interact with workshop tasks.
   
2. Use eclipse to connect to 'jdbc:hsqldb:hsql://localhost/xdb'.

3. To begin using Liquibase on the embedded database, run the following two commands:
